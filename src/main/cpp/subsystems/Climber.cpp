...
void Climber::LowerExtendingArms() {
  m_masterMotor.Set(ControlMode::PercentOutput, 0.5);
}
...
double Climber::GetLeftPosition() {
  //?: abs is needed because the slave motor is inverted
  return abs(m_slaveMotor.GetSensorCollection().GetIntegratedSensorPosition());
}
